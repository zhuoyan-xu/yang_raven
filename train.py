        ...
        for i, batch in enumerate(data_iterator):
            ...
            if not opt.use_file_passages and index_refresh_scheduler.is_time_to_refresh(step):
                ...
                if not (step == 0 and opt.load_index_path is not None):  # Dont refresh index if just loaded it
                    logger.info("Start refreshing index...")
                    ...
                    logger.info("Finish refreshing index...")

            step += 1
            train_step_start = time.time()
            
            # TODO: do we need to modify the above the chunk of code? ignore? 
            # double check: we don't need to retrieve passages here under the no retrieval scenario.

            # Index is given as the placeholder
            ...
            # TODO: Every variable 1. what it looks like 2. is it used/called

            # Index: does not sound like a index of passage, might be index of query.
            # query: what ?  [training data vs QA data], [<extra_id> ] Does it actully look like "Question: What is the capital of the ProvenceAlpes-Cote d'Azur region of France?Answer: Marseilles Answer:<extra_id_0>"
            # target: what ?  Just answer ??  
            # target_tokens: Why do we need the target tokens if we are given the target already? 
            # passage: My understanding that the is pre-retrieved passage. If we are using the pre-retrieved passage, what should it look like. A [] or "" ? 
            # batch_metadata: What is the metadata? Sounds not important, but do we need to modify it ?  
            # filtering_fun: What is the filtering function? Do we need to modify it? 
            # train_retriever: What is the train_retriever? 

            

            # Why set MLM mode vs QA mode if we don't distinguish in prompt? Check its internal implementation. 
            ...
            # TODO: What is the scale? Can you print it out? 
            ...
            if step % opt.save_freq == 0 or step == opt.total_steps:
                ...
                exit() # save the model and kill the program. Change it if your cluster has a longer time limit
