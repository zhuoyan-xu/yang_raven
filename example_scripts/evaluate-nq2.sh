...
model=atlas
# model=raven
name="nq-${model}-${size}-${n_shot}-shot-${fusion}-fusion-d${doc}-sub001"
...
SAVE_DIR=${DATA_DIR}/experiments/sub001/
EXPERIMENT_NAME=$SLURM_JOB_ID-${name}-sub001
PRECISION="bf16"
...
TEXTS="${DATA_DIR}/corpora/wiki/enwiki-dec2018/text-list-100-sec_subsample001.jsonl"
INFOBOXES="${DATA_DIR}/corpora/wiki/enwiki-dec2018/infobox_subsample001.jsonl"
PASSAGES="${TEXTS} ${INFOBOXES}"

PRETRAINED_INDEX=${DATA_DIR}/indices/wiki-mlm-${size}-atlas-org-enwiki-dec2018-sub001
...
echo "${SHARE_OUTPUT}/logs/sub001/${EXPERIMENT_NAME}_$DATETIME.log"
srun -l \
    --output ${SHARE_OUTPUT}/logs/sub001/${EXPERIMENT_NAME}_$DATETIME.log ${CMD}
