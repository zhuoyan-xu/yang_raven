...
from src.index_io import load_or_initialize_index, save_embeddings_and_index, load_or_initialize_index2, save_embeddings_and_index2
...
def freeze_layers(model):
    ...
    # Make sure shared embedding is still tied
    model.reader.lm_head.weight = model.reader.shared.weight  # Not strictly needed if already tied, but safe
    print("finished freezing layers")
...
if __name__ == "__main__":
    ...
    # freeze_layers(model) # Freeze the retriever and reader layers except for the LM head
    index2, contexts = load_or_initialize_index2(opt)
    ...
    if not opt.use_file_contexts and opt.load_index_path_data_retrieval is None:
        model.build_index2(index2, contexts, opt.per_gpu_embedder_batch_size, logger)
        if opt.save_index_path_data_retrieval is not None:
            save_embeddings_and_index2(index2, opt)
